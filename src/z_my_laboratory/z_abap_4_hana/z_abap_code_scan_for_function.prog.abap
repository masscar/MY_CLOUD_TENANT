*&---------------------------------------------------------------------*
*& Report  Z_ABAP_CODE_SCAN_FOR_FUNCTION                               *
*&                                                                     *
*&---------------------------------------------------------------------*
...
*&
*&---------------------------------------------------------------------*
REPORT z_abap_code_scan_for_function  MESSAGE-ID z_abap_4_hana
...
TYPES:
  BEGIN OF ty_output ,
    ...
    module_name                        TYPE PROGNAME ,
    line                               TYPE zasc_statement_line ,
    statement                          TYPE zasc_statement ,
    status                             TYPE zasc_statement_status ,
    funcname                           TYPE funcname ,
    notes                              TYPE zasc_notes ,
  END OF ty_output .
DATA:
  gt_output                            TYPE TABLE OF ty_output . "zst_abap_scan_tables_list .
FIELD-SYMBOLS:
...
* start scanning CALL statements
...
* check next token to contain FUNCTION
    READ TABLE gt_tokens ASSIGNING <token_nested>
      INDEX ( sy-tabix + 1 ) .
    CHECK <token_nested>-str = 'FUNCTION' .
...
* check if token following CALL FUNCTION contains single quote or not
    gv_current_row_scan = gv_current_row_scan + 2 .
    READ TABLE gt_tokens ASSIGNING <token_nested>
      INDEX gv_current_row_scan .
    IF <token_nested>-str CS '''' .
      <output>-funcname  = <token_nested>-str .
      REPLACE ALL OCCURRENCES OF '''' IN <output>-funcname WITH ''  .
      CONDENSE <output>-funcname .
      <output>-status    = ICON_SPACE .
...
* static CALL FUNCTION - we can proceed
...
* check if function name exists or not
    SELECT COUNT( * )
      FROM tfdir
      INTO gv_found
     WHERE funcname = <output>-funcname .
...
*           tadir~srcsystem = sy-sysid
    ...
    LOOP AT gt_main_modules ASSIGNING <main_module>
      ...
      IF <main_module>-generation_module = <main_module>-obj_name .
      ELSE .
        <main_module>-obj_name = <main_module>-name = <main_module>-generation_module .
      ENDIF .
    ENDLOOP .
