*&---------------------------------------------------------------------*
*& Report  Z_ABAP_CODE_RESTORE_ALL                                     *
*&                                                                     *
*&---------------------------------------------------------------------*
...
*&
*&---------------------------------------------------------------------*
REPORT z_abap_code_restore_all         MESSAGE-ID z_abap_4_hana
...
DATA:
  ...
  ls_trdir                             TYPE trdir .
DATA:
  lv_commit_frequency                  TYPE i .


  lv_commit_frequency = 0 .
  LOOP AT gt_backup_objects ASSIGNING <backup_object> .
    ...
    <output>-trkorr          = <backup_object>-trkorr .
...
*get source for restoring in case of error
    READ REPORT gs_backup_object-include_program INTO gt_abap_lines .


    CASE <backup_object>-object .
      WHEN 'FUNC' .
        SELECT SINGLE *
          FROM trdir
          INTO ls_trdir
        ...
        INSERT REPORT gs_backup_object-include_program FROM gt_abap_lines_backup
      ...
      WHEN Others .
        ...
        <output>-notes  = |{ <backup_object>-object }-{ <backup_object>-obj_name } UNKNOWN| .
        CONTINUE .
    ...
    ELSE .
      <output>-status = ICON_LED_RED .
      <output>-notes  = |{ lv_line }: { lv_message }| .
    ...
    lv_commit_frequency = lv_commit_frequency + 1 .
    IF lv_commit_frequency = 500 .
...
DATA:
  ...
  lv_object_type = gs_backup_key-object .
  lv_object_name = gs_backup_key-obj_name .
