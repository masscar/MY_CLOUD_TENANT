...
DATA:
  ...
  TRY .
    lo_producer_pcp ?= cl_amc_channel_manager=>create_message_producer(
      i_application_id = 'Z_AMC_MESSAGE'
      i_channel_id     = '/message_from_backend_pcp'
    ) .

*    lo_producer_text->send( i_message = lv_message ) .

    lo_pcp_message ?= cl_ac_message_type_pcp=>create( ) .
    lo_pcp_message->set_field( i_name  = 'CODICE'   i_value = '10' ) .
    lo_producer_pcp->send( i_message = lo_pcp_message ) .
